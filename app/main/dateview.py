...
class DateView(BaseView):
    ...
    def index(self):
        ...
        xls = pd.ExcelFile('out_completed_tasks_d.xls')
        completed_tasks_df = pd.read_excel(xls)
        xls = pd.ExcelFile('out_incoming_tasks_d.xls')
        incoming_tasks_df = pd.read_excel(xls)
        xls = pd.ExcelFile('out_backlog_tasks_d.xls')
        ...
        dates = list(dict.fromkeys(completed_tasks_df['Date']))

        completed_tasks_df_g = completed_tasks_df.groupby(['Year', 'Date', 'Task_Type_Ref'], as_index=False, sort=False).sum()
        incoming_tasks_df_g = incoming_tasks_df.groupby(['Year', 'Date', 'Task_Type_Ref'], as_index=False, sort=False).sum()
        backlog_tasks_df_g = backlog_tasks_df.groupby(['Year', 'Backlog_Date', 'Task_Type_Ref']).sum()
        ...
        callback_co = CustomJS(args={'source01': source01, 'source_completed_tasks_df': source_completed_tasks_df, 'source_incoming_tasks_df': source_incoming_tasks_df, 'source_backlog_tasks_df': source_backlog_tasks_df,
            ...
            for (var i = 0; i < dates.length; i++){
                ...
                for (var j = 0; j < completed_tasks.central_office.length; j++) {
                    if ( (completed_tasks.Date[j] == dates[i]) && (this.value.includes(completed_tasks.central_office[j])) ) {
                ...
                for (var j = 0; j < incoming_tasks.central_office.length; j++) {
                    if ( (incoming_tasks.Date[j] == dates[i]) && (this.value.includes(incoming_tasks.central_office[j])) ) {
                ...
                for (var j = 0; j < backlog_tasks.central_office.length; j++) {
                    if ( (backlog_tasks.Backlog_Date[j] == dates[i]) && (this.value.includes(backlog_tasks.central_office[j])) ) {
        ...
        callback_org = CustomJS(args={'source01': source01, 'source_completed_tasks_df': source_completed_tasks_df, 'source_incoming_tasks_df': source_incoming_tasks_df, 'source_backlog_tasks_df': source_backlog_tasks_df,
            ...
            for (var i = 0; i < dates.length; i++){
                ...
                for (var j = 0; j < completed_tasks.organisation.length; j++) {
                    if ( (completed_tasks.Date[j] == dates[i]) && (this.value.includes(completed_tasks.organisation[j])) ) {
                        if (!co_options.includes(completed_tasks.central_office[j])) {
                            co_options.push(completed_tasks.central_office[j]);
                        }
                        //if (!org_options.includes(completed_tasks.organisation[j])) {
                ...
                for (var j = 0; j < incoming_tasks.organisation.length; j++) {
                    if ( (incoming_tasks.Date[j] == dates[i]) && (this.value.includes(incoming_tasks.organisation[j])) ) {
                ...
                for (var j = 0; j < backlog_tasks.organisation.length; j++) {
                    if ( (backlog_tasks.Backlog_Date[j] == dates[i]) && (this.value.includes(backlog_tasks.organisation[j])) ) {
        ...
        callback_users = CustomJS(args={'source01': source01, 'source_completed_tasks_df': source_completed_tasks_df, 'source_incoming_tasks_df': source_incoming_tasks_df, 'source_backlog_tasks_df': source_backlog_tasks_df,
            ...
            for (var i = 0; i < dates.length; i++){
                ...
                for (var j = 0; j < completed_tasks.user.length; j++) {
                    if ( (completed_tasks.Date[j] == dates[i]) && (this.value.includes(completed_tasks.user[j])) ) {
                        if (!co_options.includes(completed_tasks.central_office[j])) {
                            co_options.push(completed_tasks.central_office[j]);
                        }
                        if (!org_options.includes(completed_tasks.organisation[j])) {
                ...
                for (var j = 0; j < incoming_tasks.user.length; j++) {
                    if ( (incoming_tasks.Date[j] == dates[i]) && (this.value.includes(incoming_tasks.user[j])) ) {
                        if (!co_options.includes(incoming_tasks.central_office[j])) {
                            co_options.push(incoming_tasks.central_office[j]);
                        }
                        if (!org_options.includes(incoming_tasks.organisation[j])) {
                ...
                for (var j = 0; j < backlog_tasks.user.length; j++) {
                    if ( (backlog_tasks.Backlog_Date[j] == dates[i]) && (this.value.includes(backlog_tasks.user[j])) ) {
                        if (!co_options.includes(backlog_tasks.central_office[j])) {
                            co_options.push(backlog_tasks.central_office[j]);
                        }
                        if (!org_options.includes(backlog_tasks.organisation[j])) {
        ...
        callback_categories = CustomJS(args={'source01': source01, 'source_completed_tasks_df': source_completed_tasks_df, 'source_incoming_tasks_df': source_incoming_tasks_df, 'source_backlog_tasks_df': source_backlog_tasks_df,
            ...
            for (var i = 0; i < dates.length; i++){
                ...
                for (var j = 0; j < completed_tasks.category.length; j++) {
                    if ( (completed_tasks.Date[j] == dates[i]) && (completed_tasks.category[j] != 0) && (this.value.includes(completed_tasks.category[j].substring(14))) ) {
                        if (!co_options.includes(completed_tasks.central_office[j])) {
                            co_options.push(completed_tasks.central_office[j]);
                        }
                        if (!org_options.includes(completed_tasks.organisation[j])) {
                ...
                for (var j = 0; j < incoming_tasks.category.length; j++) {
                    if ( (incoming_tasks.Date[j] == dates[i]) && (incoming_tasks.category[j] != 0) && (this.value.includes(incoming_tasks.category[j].substring(14))) ) {
                        if (!co_options.includes(incoming_tasks.central_office[j])) {
                            co_options.push(incoming_tasks.central_office[j]);
                        }
                        if (!org_options.includes(incoming_tasks.organisation[j])) {
                ...
                for (var j = 0; j < backlog_tasks.category.length; j++) {
                    if ( (backlog_tasks.Backlog_Date[j] == dates[i]) && (backlog_tasks.category[j] != 0) && (this.value.includes(backlog_tasks.category[j].substring(14))) ) {
                        if (!co_options.includes(backlog_tasks.central_office[j])) {
                            co_options.push(backlog_tasks.central_office[j]);
                        }
                        if (!org_options.includes(backlog_tasks.organisation[j])) {
        ...
        p1 = figure(x_range=dates, plot_width=1000, plot_height=500, title="Tasks completed by date",
                   toolbar_location=None, tools="")

        r_ss = p1.vbar(x=dodge('dates', -0.3, range=p1.x_range), top='Site_Surveys', width=0.18, source=source01,
                      color="#4B0082")

        hover_ss = HoverTool(tooltips="Site Surveys: @Site_Surveys", renderers=[r_ss])

        r_ic = p1.vbar(x=dodge('dates', -0.1, range=p1.x_range), top='Infrastructure_Constructions', width=0.18,
        ...
        r_onc = p1.vbar(x=dodge('dates', 0.1, range=p1.x_range), top='Opt_Network_Constructions', width=0.18,
        ...
        r_cc = p1.vbar(x=dodge('dates', 0.3, range=p1.x_range), top='Customer_Connections', width=0.18, source=source01,
                      color="#006400")
        ...
        p2 = figure(x_range=dates, plot_width=1000, plot_height=420, title="Incoming vs completed orders by date",
                   toolbar_location=None, tools="")

        r_io = p2.vbar(x=dodge('dates', -0.15, range=p2.x_range), top='Incoming_Orders', width=0.28, source=source01,
                      color="#4B0082")

        hover_io = HoverTool(tooltips="Incoming Orders: @Incoming_Orders", renderers=[r_io])

        r_co = p2.vbar(x=dodge('dates', 0.15, range=p2.x_range), top='Customer_Connections', width=0.28, source=source01,
                      color="#006400")
        ...
        p3 = figure(x_range=dates, plot_width=1000, plot_height=370, title="Backlog of orders by date", toolbar_location=None, tools="")

        r_bo = p3.vbar(x=dodge('dates', 0.0, range=p3.x_range), top='Backlog_Orders', width=0.4, source=source01,
                      color="#e84d60")
